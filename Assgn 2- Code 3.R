...
B = 10000 #assigning value of B for Boosting
r_train = y_train
fx = matrix(ncol =4, nrow = B)
eta = 0.01
for (b in 1:B){
    best_error = Inf
    best_split = 0
    ...
    #fx is B*4 matrix which stores best_feature, best_split, and mean values for both branches after the split    
    fx[b,1] = best_feature
    fx[b,2] = best_split
    fx[b,3] = best_m1
    fx[b,4] = best_m2
...
y_pred = matrix(nrow= nrow(X_test),ncol = B)
for (b in 1:B){                                     #calculating prediction values for each of the B stumps for the test set
    if(fx[b,1]==1){
...
#calculating test error for B stumps where B is ranging from 1 to 10000(or any other value of B)
tse = replicate(B, 0)
y_hat = matrix(nrow= nrow(X_test),ncol = 2)
y_hat[,1] = y_pred[,1]
tse[1] = mean((y_test - eta*y_pred[,1])^2)
for (f in 2:B){
    y_hat[,2] = y_hat[,1]+y_pred[,f]
    tse[f] = mean((y_test - eta*y_hat[,2])^2)
    y_hat[,1] = y_hat[,2]
    
}

plot(tse , type ='l',ylim=c(20,45),xlab="B", ylab="Test MSE", main = 'Test MSE vs B')
